...
- Use `app/src/` for Next.js application source code
- Use `app/` for all Node.js config (package.json, tsconfig, etc)
- Use `browser/` for Chrome extension source
- Use `agent/` for Claude skill code (deployed to .claude/linkedin-prospector)
- Use `tests/` for test files (jest runs from app/, paths resolve via ../tests)
...
- Clean `.next` via `docker exec ctox-app rm -rf /app/.next` if root-owned files block local rm
